(* M12: the span framework.  This module owns candidate hygiene,
   ...
   decoded tree.  The detectors themselves are M13..M17: a [matcher]
   is the plug-in point, and [matchers] is empty until they land, so
   every production entry point here is the identity today.
